...
public class ProyectoUsuarioDao {

    private static final Logger LOGGER = Logger.getLogger(ProyectoUsuarioDao.class.getName());
    private final Connection connection;

    public ProyectoUsuarioDao() {
    ...
    public boolean asignarUsuarioAProyecto(ProyectoUsuario pu) {
        ...
        String sql = "INSERT INTO proyecto_usuario (proyecto_id, usuario_id, rol) VALUES (?, ?, ?)";
    ...
    public ProyectoUsuario obtenerAsignacion(int proyectoId, int usuarioId) {
        String sql = "SELECT * FROM proyecto_usuario WHERE proyecto_id = ? AND usuario_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public boolean actualizarRol(ProyectoUsuario pu) {
        ...
        String sql = "UPDATE proyecto_usuario SET rol = ? WHERE proyecto_id = ? AND usuario_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public boolean eliminarAsignacion(int proyectoId, int usuarioId) {
        ...
        String sql = "DELETE FROM proyecto_usuario WHERE proyecto_id = ? AND usuario_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, proyectoId);
            stmt.setInt(2, usuarioId);
            return stmt.executeUpdate() > 0;
    ...
    public ObservableList<ProyectoUsuario> obtenerUsuariosDeProyecto(int proyectoId) {
        ...
        String sql = "SELECT * FROM proyecto_usuario WHERE proyecto_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, proyectoId);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
    ...
    public ObservableList<ProyectoUsuario> obtenerUsuariosDeProyectoPaginado(int proyectoId, int limit, int offset) {
        ObservableList<ProyectoUsuario> lista = FXCollections.observableArrayList();
        String sql = "SELECT * FROM proyecto_usuario WHERE proyecto_id = ? ORDER BY usuario_id LIMIT ? OFFSET ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, proyectoId);
            stmt.setInt(2, limit);
    ...
    public ObservableList<ProyectoUsuario> obtenerProyectosDeUsuarioPaginado(int usuarioId, int limit, int offset) {
        ObservableList<ProyectoUsuario> lista = FXCollections.observableArrayList();
        String sql = "SELECT * FROM proyecto_usuario WHERE usuario_id = ? ORDER BY proyecto_id LIMIT ? OFFSET ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public ObservableList<ProyectoUsuario> obtenerProyectosDeUsuario(int usuarioId) {
        ObservableList<ProyectoUsuario> lista = FXCollections.observableArrayList();
        String sql = "SELECT * FROM proyecto_usuario WHERE usuario_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, usuarioId);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
    ...
    private boolean existeAsignacion(int proyectoId, int usuarioId) {
        String sql = "SELECT 1 FROM proyecto_usuario WHERE proyecto_id = ? AND usuario_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, proyectoId);
            stmt.setInt(2, usuarioId);
            ResultSet rs = stmt.executeQuery();
